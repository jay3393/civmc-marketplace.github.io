...
export default function ShopDetailPage() {
  ...
  async function addListing() {
    if (!isOwner || !outputItemName.trim() || !inputItemName.trim()) return;
    const sb = getSupabaseBrowser();
    const payload = {
      shop_id: shopId,
      input_item_id: null,
      input_item_name: inputItemName.trim(),
      input_qty: Number(inputQty) || 1,
    ...
    if (!error && data) {
      ...
      setInputQty(1); setOutputQty(1); setNewStock(""); setNewNotes("");
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Items grid (owner can manage) */}
      {isOwner ? (
        <div className="grid gap-4">
          <div className="rounded-xl border p-3 grid gap-3 max-w-2xl">
            <div className="text-sm font-semibold">Add exchange listing</div>
            <div className="grid gap-2">
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                <div className="grid gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Input type="number" min={0} value={inputQty} onChange={(e) => setInputQty(Number(e.target.value) || 0)} className="h-8 w-20" />
                  </div>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div className="grid gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    <label className="text-[11px] text-muted-foreground">Qty</label>
                    <Input type="number" value={outputQty} min={0} onChange={(e) => setOutputQty(Number(e.target.value) || 0)} className="h-8 w-20" />
                  </div>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <div className="grid gap-1">
              ...
              <Textarea placeholder="Enchantment details, notes…" value={newNotes} onChange={(e) => setNewNotes(e.target.value)} />
            </div>
            <div>
              <Button onClick={addListing} disabled={!outputItemName.trim() || !inputItemName.trim()}>Add listing</Button>
            </div>
          </div>

          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <div key={item.id} className="rounded-lg border p-3 grid gap-3">
                <div className="grid gap-2">
                  <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                    <div className="grid gap-1">
                      <label className="text-xs text-muted-foreground">Input (what players give)</label>
                      <input list="item-catalog" className="border rounded-md h-9 px-2 text-sm" placeholder="Search or type item…" value={item.inputName} onChange={(e) => updateListing(item.id, { inputName: e.target.value })} />
                      <div className="flex items-center gap-2">
                        <label className="text-[11px] text-muted-foreground">Qty</label>
                        <Input type="number" value={item.inputQty ?? 1} onChange={(e) => updateListing(item.id, { inputQty: Number(e.target.value) || 1 })} className="h-8 w-20" />
                      </div>
                    </div>
                    <div className="text-center text-muted-foreground">⇄</div>
                    <div className="grid gap-1">
                      <label className="text-xs text-muted-foreground">Output (what players receive)</label>
                      <input list="item-catalog" className="border rounded-md h-9 px-2 text-sm" placeholder="Search or type item…" value={item.outputName} onChange={(e) => updateListing(item.id, { outputName: e.target.value })} />
                      <div className="flex items-center gap-2">
                        <label className="text-[11px] text-muted-foreground">Qty</label>
                        <Input type="number" value={item.outputQty ?? 1} onChange={(e) => updateListing(item.id, { outputQty: Number(e.target.value) || 1 })} className="h-8 w-20" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <Input type="number" placeholder="Stock" value={item.stock ?? 0} onChange={(e) => updateListing(item.id, { stock: Number(e.target.value) || 0 })} />
                  <button className={`h-8 rounded-md border px-2 text-xs ${item.is_listed ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-red-50 text-red-700"}`} onClick={() => updateListing(item.id, { is_listed: !item.is_listed })}>
                    {item.is_listed ? "Listed" : "Unlisted"}
                  </button>
                </div>
                <Textarea placeholder="Description (optional)" value={item.notes ?? ""} onChange={(e) => updateListing(item.id, { notes: e.target.value || null })} />
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">Rate: {item.inputQty} {item.inputName} ⇄ {item.outputQty} {item.outputName}</span>
                  <Button variant="outline" className="h-8" onClick={() => deleteListing(item.id)}>Delete</Button>
                </div>
              </div>
            ))}
          </div>
